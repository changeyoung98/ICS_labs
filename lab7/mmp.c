...
{
    int rootOffSet; 
    int index = find_list_root(GET_SIZE(HDRP(p)));
    index += block_list_start;
    rootOffSet=GET(index);
   if(rootOffSet==0){
    ...
    {
        PUT(NEXT_LINKNODE_RP(prevp),GET(p));
        PUT(PREV_LINKNODE_RP(p),GET(prevp));
        PUT(NEXT_LINKNODE_RP(p),GET(nextp));
        if(nextp!=NULL) PUT(PREV_LINKNODE_RP(nextp),GET(p));
    }
  }
