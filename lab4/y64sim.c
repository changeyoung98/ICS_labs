...
{
    bool_t doit = FALSE;
    if((GET_ZF(cc)==1)&&(cond==C_E)){
       doit=TRUE;
    }
    if((cond==C_NE)&&(GET_ZF(cc)==0)){
       doit=TRUE;
    }
    if((cond==C_LE)&&((GET_ZF(cc)|(GET_SF(cc)^GET_OF(cc)))==1)){
       doit=TRUE;
    }
    if((cond==C_GE)&&((GET_SF(cc)^GET_OF(cc))==0)){
       doit=TRUE;
    }
    if((cond==C_L)&&((GET_SF(cc)^GET_OF(cc))==1)){
       doit=TRUE;
    }
    if((cond==C_G)&&((GET_ZF(cc)|(GET_SF(cc)^GET_OF(cc)))==0)){
       doit=TRUE;
    }
    if(cond==C_YES){
       doit=TRUE;
...
stat_t nexti(y64sim_t *sim)
{
    ...
    switch (icode) {
      ...
      case I_MRMOVQ: /* 5:0 regB:regA imm */
        ...
        valB=get_reg_val(sim->r,regB);
        addr=valB+immediate;
       // c=compute_cc(A_ADD,valB,immediate,addr);
        if (!get_long_val(sim->m,addr,&val)){
            err_print("%s0x%x, %s 0x%lx","PC = ",sim->pc,"Invalid data address",addr);
            return STAT_ADR;
        }  
      ...
      case I_ALU: /* 6:x regA:regB */
        ...
        c=compute_cc(ifun,valA,valB,val);
        sim->cc=c;
        sim->pc = next_pc;
        break;
      ...
      case I_JMP: /* 7:x imm */
        ...
        break;
      case I_CALL: /* 8:x imm */
        sp=get_reg_val(sim->r,REG_RSP);
        sp=sp-8;
        set_reg_val(sim->r,REG_RSP,sp);
        if(!set_long_val(sim->m,sp,next_pc)){
           err_print("%s0x%x, %s 0x%lx","PC = ",sim->pc,"Invalid stack address",sp);
            return STAT_ADR;
        }
        sim->pc=immediate;
        break;
      ...
      case I_RET: /* 9:0 */
        sp=get_reg_val(sim->r,REG_RSP);
        val=0;
        if (!get_long_val(sim->m,sp,&val)){
           err_print("%s0x%x, %s 0x%lx","PC = ",sim->pc,"Invalid data address",sp);
           return STAT_ADR;
