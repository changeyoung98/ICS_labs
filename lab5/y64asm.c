...
int add_symbol(char *name)
{
    ...
    symbol_t *nsymbol=(symbol_t*)malloc(sizeof(symbol_t));
    memset(nsymbol,0,sizeof(symbol_t));
    nsymbol->name=name;
    nsymbol->addr=vmaddr;
...
void add_reloc(char *name, bin_t *bin)
{
    /* create new reloc_t (don't forget to free it)*/
       reloc_t *nreloc=(reloc_t *)malloc(sizeof(reloc_t));
       memset(nreloc,0,sizeof(reloc_t));
       nreloc->name=name;
       nreloc->y64bin=bin;   
...
parse_t parse_symbol(char **ptr, char **name)
{
    /* skip the blank and check */
      ...
      int i=0;
      while((!IS_BLANK(*ptr+i))&&(!IS_END(*ptr+i))&&((IS_LETTER(*ptr+i))||(IS_DIGIT(*ptr+i)))){
          i++;
      }
      if(i!=0){
    /* allocate name and copy to it */
      *name=(char*)malloc(i*sizeof(char));
      memset(*name,'\0',i*sizeof(char));
      strncpy(*name,*ptr,i);
    /* set 'ptr' and 'name' */
     *ptr+=i;
      return PARSE_SYMBOL;
...
parse_t parse_imm(char **ptr, char **name, long *value)
{
    ...
    /* if IS_IMM, then parse the digit */
       if(IS_IMM(*ptr)){
          ...
          par= parse_digit(ptr,value);
       }
    /* if IS_LETTER, then parse the symbol */
        if(IS_LETTER(*ptr)){
          par= parse_symbol(ptr,name);
       }
   ...
   /* parse the rest of instruction according to the itype */
       ...
       switch(HIGH(inst->code)){
           ...
           case I_IRMOVQ: 
              ...
              if(par==PARSE_DIGIT){
                  memcpy(line->y64bin.codes+2,(void*)&val,sizeof(long));
              }
              if(par==PARSE_SYMBOL){
           ...
           case I_RMMOVQ:
              if(parse_reg(&ptr,&regA)==PARSE_ERR){
                ...
                if(parse_mem(&ptr,&val,&regB)==PARSE_ERR){
                    line->type=TYPE_ERR;
                    return line->type;
               }
               line->y64bin.codes[1]=HPACK(regA,regB);
          ...
          case I_JMP:
          case I_CALL:
              if(parse_symbol(&ptr,&sym)==PARSE_SYMBOL){
                   add_reloc(sym,&line->y64bin);
              }
              else{
         ...
         case I_DIRECTIVE:
              switch(LOW(inst->code)){
                  case D_DATA:
                      ...
                      if(par==PARSE_DIGIT){
                           memcpy(line->y64bin.codes,(void*)&val,inst->bytes);
                      }
                      if(par==PARSE_ERR){
                            line->type=TYPE_ERR;
                           return line->type;
...
int relocate(void)
{
    ...
    while (rel) {
        /* find symbol */
         symbol_t * sym= find_symbol(rel->name);
         if(sym==NULL){
            err_print("Unknown symbol:'%s'",rel->name);
           return -1;
         }
        /* relocate y64bin according itype */
         int type=rel->y64bin->bytes-8;
         memcpy(rel->y64bin->codes+type,(void*)&sym->addr,sizeof(int));
        /* next */
        rel = rel->next;
    }
    return 0;
...
int binfile(FILE *out)
{
    ...
    /* prepare image with y64 binary code */
      ...
      char* image=(char*)malloc(vmaddr*sizeof(char));
      memset(image,HPACK(0,0),vmaddr*sizeof(char));
      int bytes_sum=0;
      while(line->next!=NULL){
        line=line->next;
        if(line->type==TYPE_INS){
           int bytes=line->y64bin.bytes;
           int i;
           for(i=0;i<bytes;i++){
               strncpy(&image[line->y64bin.addr]+i,(char*)(line->y64bin.codes)+i,sizeof(char));
          
         }
          if(line->y64bin.bytes){
             bytes_sum=line->y64bin.addr+line->y64bin.bytes;
          }
        }
      }
    /* binary write y64 code to output file (NOTE: see fwrite()) */
       fwrite(image,sizeof(char),bytes_sum,out);
