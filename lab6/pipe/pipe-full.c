...
long long gen_f_pc()
{
    return (((((((if_id_next->icode) == (I_JMP)) & ((if_id_next->ifun) == 
                (C_LE))) & ((id_ex_curr->icode) == (I_ALU))) & (
            (id_ex_curr->ifun) == (A_AND))) & ((id_ex_curr->vala) > 0)) ? 
      (if_id_next->valp) : (((((ex_mem_curr->icode) == (I_JMP)) & (
              (ex_mem_curr->ifun) == (C_LE))) & ((mem_wb_curr->icode) == 
            (I_ALU))) & ((mem_wb_curr->ifun) == (A_AND))) ? (pc_curr->pc)
...
long long gen_d_valA()
{
    return ((((((if_id_curr->icode) == (I_JMP)) & ((if_id_curr->ifun) == 
              (C_LE))) & ((id_ex_curr->icode) == (I_ALU))) & (
          (id_ex_curr->ifun) == (A_AND))) ? (d_regvala) : (
...
long long gen_D_bubble()
{
    return ((!(((((id_ex_curr->icode) == (I_JMP)) & ((id_ex_curr->ifun) == 
                (C_LE))) & ((ex_mem_curr->icode) == (I_ALU))) & (
            (ex_mem_curr->ifun) == (A_AND))) & (((id_ex_curr->icode) == 
            (I_JMP)) & !(ex_mem_next->takebranch))) | (!((
