...
# 1.I added the iaddq function
# 2.There are lots of combinations of andq and jle, so I decide on the 
# value of E_valA when f_pc fetch the next PC, therefore, it cause no
# bubble
####################################################################
...
wordsig d_icode   'id_ex_next->icode'
wordsig d_ifun   'id_ex_next->ifun'
...
word f_pc = [
       f_icode ==IJXX && f_ifun==CJLE && E_icode==IOPQ && E_ifun==ALUAND && (E_valA>0):f_valP;
	# Mispredicted branch.  Fetch at incremented PC
        M_icode == IJXX&&M_ifun==CJLE &&W_icode==IOPQ&&W_ifun==ALUAND: F_predPC;
...
word d_valA = [
        D_icode==IJXX&&D_ifun==CJLE&&E_icode==IOPQ&&E_ifun==ALUAND:d_rvalA;
...
bool D_bubble =
	# Mispredicted branch
	!(E_icode==IJXX&&E_ifun==CJLE&&M_icode==IOPQ&&M_ifun==ALUAND)&&(E_icode == IJXX && !e_Cnd) ||
	# Stalling at fetch while ret passes through pipeline
