...
void serve_proxy(clientinfo_t clientinfo, rio_t *rio, char *uri, char *method, char *hostname, char *pathname, int port, char *version,char* headers);
 ...
 */
void doit(clientinfo_t clientinfo){
    char buf[MAXLINE],headers[MAXBUF], method[MAXLINE], uri[MAXLINE], hostname[MAXLINE], pathname[MAXLINE], version[MAXLINE];
    int port;
    rio_t rio;
    ...
    if(parse_uri(uri, hostname, pathname, &port) < 0){
     ...
     process_hdrs(&rio, headers, hostname);
    printf("%s %s\n", hostname, pathname);
    serve_proxy(clientinfo, &rio, uri, method, hostname, pathname, port, version,headers);
}
...
void serve_proxy(clientinfo_t clientinfo, rio_t *rio, char *uri, char *method, char *hostname, char *pathname, int port, char *version,char* headers){
    int serverfd, bytes;
    int tot_bytes = 0;
    char request_buf[MAXLINE],response_buf[MAXLINE], log_buffer[MAXLINE],buf[MAXLINE];
    char hdrstr[MAXBUF];
    rio_t response_rio;
    ...
    sprintf(response_buf,"GET HTTP/1.1 200 OK");
