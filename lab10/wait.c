...
void *doit(void *vargp)
{
    ...
    char buf_server[MAXLINE],buf_client[MAXLINE]; /* Line buffer */
   ...
   // int reach_body = 0; /* Whether body has begun */
    ...
    if (Rio_readlineb_w(&rio_conn, buf_server, MAXLINE) == 0) {
        Close(connfd);
        return NULL;
    }
...
//clientfd=Open_clientfd_ts(hostname,port);
  ...
  //  Rio_readinitb(&rio_client, clientfd);
   ...
   // Rio_writen_w(clientfd,headers,strlen(headers));

    /* Forward remaining request lines to target server */
    while ((n = Rio_readlineb_w(&rio_conn, buf_server, MAXLINE)) != 0) {
        Rio_writen_w(clientfd, buf_server, n);
        if (!strncasecmp(buf_server, "Content-length:", 15)) {
           if(sscanf(buf_server + 15, "%d", &body_len)!=1){
               Close(clientfd);
    			Close(connfd);
				return NULL;
           };
...
if(body_len_set){
   ...
   for (i=0;i<body_len;i++){
			n=Rio_readnb_w(&rio_conn,buf_server,1);    
			if (n!=1) {
				Close(clientfd);
				Close(connfd);
				return NULL;
    ...
    n=Rio_readlineb_w(&rio_client, buf_client, MAXLINE);
    if (n==0){
      Close(clientfd);
      Close(connfd);
      return NULL;
    ...
    while (strcmp(buf_client,"\r\n")) {
        ...
        recv_len += n;
       if (!body_len_set && !strncasecmp(buf_client, "Content-length:", 15)) {
          if(sscanf(buf_client + 15, "%d", &body_len)!=1){
              Close(clientfd);
    			Close(connfd);
				return NULL;
          };
...
/*body*/
     while (n=Rio_readnb_w(&rio_client, buf_client, 1)!=0){
        /* Calculate length and detect body */
        if (n == MAXLINE && buf_client[MAXLINE - 2] != '\n')
            --n;
        Rio_writen_w(connfd, buf_client, 1);
